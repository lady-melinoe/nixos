  ...
  systemd.services.melinoe-inet-setup =
    let
      ...
      setupScript = pkgs.writeShellScript "melinoe-inet-setup" ''
        ...
        ip netns exec inet ip addr replace 198.19.1.5/24 dev ens3
        ...
        ip netns exec inet ip route add default via 198.19.1.1 dev ens3 
        ip netns exec inet iptables -t nat -C PREROUTING -d 198.19.1.5 -j DNAT --to-destination 198.18.0.5
        ip netns exec inet iptables -t nat -A PREROUTING -d 198.19.1.5 -j DNAT --to-destination 198.18.0.5
