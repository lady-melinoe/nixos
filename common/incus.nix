  ...
  incusSnapshotScript = pkgs.writeTextFile {
    ...
    text = ''
      ...
      def ingest_snapshot(container_name: str, repo_path: Path, dest: Path) -> bool:
          ...
          subprocess.run(
              ["btrfs", "subvolume", "delete", str(dest / container_name)],
              check=True,
          )
      ...
      def ingest_pending_snapshots(container_name: str, repo_path: Path) -> None:
          ...
          if had_failure:
              print(
                  f"One or more snapshots for {container_name} failed and will be retried next cycle"
              )
      ...
      def run_once() -> None:
          ...
          for entry in SRC.iterdir():
              ...
              thread = threading.Thread(
                  target=snapshot_container, args=(entry, timestamp)
              )
              thread.start()
